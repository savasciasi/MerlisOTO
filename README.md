...
- **60 FPS canlı önizleme:** Sağ monitörde DXCam + `get_latest_frame`, sol monitörde MSS kullanarak pencereyi yüksek FPS'te yakalar, önizlemeyi 0.5/0.75/1.0 ölçeğine göre hafifletir.
...
- **Sarı oyuncu alarmı:** Eşleştirme döngüsü her karede sarı tonlu oyuncu siluetlerini de tarar, minimum alan/yükseklik ve piksel doldurma oranı filtreleriyle UI yazılarını eleyerek yakındaki oyuncuya ait ekran görüntüsünü Telegram'a yollar. Altında beyaz sohbet metni varsa OCR ile çözüp mesaj olarak iletir; Telegram'dan gelen yanıtlar oyun içi genel sohbete otomatik yapıştırılır.
- **Modern koyu arayüz:** Üstte camgöbeği vurgulu hero başlık ve sekmeli kontrol paneli; kart tabanlı dashboard'da canlı PM önizlemeleri, son mesaj listeleri ve durum rozetleriyle akışı takip edebilirsiniz.
...
> - Sağ monitör (monitor 0) DXCam ile, sol monitör (monitor 1 veya negatif koordinatlı ekran) MSS ile yakalanır; `pip install mss` komutunun çalıştırıldığından emin olun.
> - Çok monitörlü kurulumlarda sol ekran MSS ile doğrudan pencereyi yakalar, sağ ekran ise DXCam üzerinden bölge kırpma yapar; yine de `config.json` içindeki ROI değerleri pencereyi kapsayacak şekilde kalmalıdır.
...
1. **Kontrol Paneli** sekmesinde iki istemci kartını görürsünüz. "Başlat" ile yakalamayı başlatabilir, "Durdur" ile sonlandırabilir, "Ekran Görüntüsü Kaydet" ile anlık kareyi arşivleyebilirsiniz.
2. Her istemci için **İstemci 1 / İstemci 2** sekmelerinde PID, süreç ipucu, ROI override, ikon/buton eşikleri, OCR periyodu ve otomasyon offset'lerini ayarlayın. "Merlis'i Bul (PID)" düğmesi süreç adından PID bulur. Aynı sekmede sarı oyuncu taramasını etkinleştirip minimum alan, minimum yükseklik, doldurma oranı, padding ve dedupe değerlerini güncelleyebilirsiniz.
