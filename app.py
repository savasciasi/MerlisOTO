...
class CaptureWorker(QThread):
    ...
    def __init__(
        ...
        self._capture_backend: str = "DXCam"
    ...
    def run(self) -> None:  # noqa: D401
        ...
        try:
            backend_name, capture, monitor_idx = select_capture_backend(
                region=region,
                prefer_dx=bool(self.client_config.get("dx_prefer", True)),
                target_fps=int(self.client_config.get("target_fps", 60)),
            )
            self._capture_backend = backend_name
            self._capture = capture
            self._capture.start()
            monitor_label = f"monitor {monitor_idx}" if monitor_idx is not None else "varsayılan ekran"
            self.logMessage.emit(
                self.client_index,
                "INFO",
                f"{backend_name} yakalama başlatılıyor · {monitor_label}.",
            )
            ...
            self.logMessage.emit(self.client_index, "ERROR", f"Yakalama başlatılamadı: {message}")
            return
